    ...
    반복문 
        ...
        - 컴퓨터가 읽어드리는 흐름순서
            1. 초기값 -> 2. 조건문(true) -> 3.실행문 -> 4. 증감식 -> 5. 조건문(true) -> 6.실행문
                                                    . 증감식 -> 5. 조건문(true) -> 6.실행문
                                                    . 증감식 -> 5. 조건문(true) -> 6.실행문
                                                    . 증감식 -> 5. 조건문(false) -> 종료
        
        
         - 주체(누가) 어디서부터 어디까지 증감하면서 반복하는 지
         예]
...
for( int 반복변수 = 1; 반복변수 <=5 ; 반복변수++ ) {
        console.log(반복변수);
}
    //i = iterater 기본적으로 예시로 i를 씀

//[5] 1부터 10까지 누적 합계를 계산하시오.
//처음에는 [반복문 없이] 함수있는상태와 없는상태를 비교해서 봐보기 

let 총합계 = 0;          // 총합계 라는 변수에 0 대입
                            // 총합계 = 총합계 + 값; vs 총합계 +=값; 둘다 가능 잘되면 오른쪽 쓰기

총합계 = 총합계 + 1;        // 총합계 변수를 호출해서 +1 한 결과를 총합계에 대합, 총합계 1
총합계 = 총합계 + 2;        
총합계 = 총합계 + 3;        
총합계 = 총합계 + 4;        
총합계 = 총합계 + 5;       
...
// 반복도 되지않는 코드 순서/ 패턴 : 1부터 10까지 1씩 증가
//[반복문 코드]

let sum = 0;
for( let 숫자패턴 =1 ; 숫자패턴 <= 10 ; 숫자패턴 ++){ // 숫자는 1부터 10이하까지 1씩 증가 반복
    총합계 = 총합계 + 숫자패턴
}// for end
...
// 반복되는 코드 : comsole.log (`2 x ? = ${2*?}`);
// 반복되지 않는 코드 : 1 2 3 4 5 6 7 8 9
// 반복되지 않는 코드 순서/패턴 : 1부터 9까지 1씩 증가 반복
//[반복문 코드]
for(let 곱 = 1; 곱 <= 9; 곱++) {// 곱은 1부터 곱은 9 이하까지 곱은 1씩 증가 반복
    comsole.log (`2 x ${곱} = ${2*곱}`);
}
